...
export default function App() {
  ...
  const currentSubtab = activeSubtab && currentTab
    ? currentTab.subtabs.find(s => s.id === activeSubtab)
    : null;
  ...
  const updateCtx = (fn) => {
    setTabs(prev => prev.map(t => {
      ...
      if (currentSubtab) {
        return { ...t, subtabs: t.subtabs.map(s => s.id === activeSubtab ? fn(s) : s) };
      }
      return fn(t);
    }));
  };
  ...
  const addTab = () => {
    ...
    setTabs(p => [...p, { id, label: name, color, subtabs: [], tasks: [], reminders: [] }]);
    setNewTabInput(""); setShowNewTab(false);
    setActiveTab(id); setActiveSubtab(null);
  };

  const deleteTab = (id) => {
    setTabs(p => p.filter(t => t.id !== id));
    if (activeTab === id) { setActiveTab(null); setActiveSubtab(null); }
  };
  ...
  const addSubtab = () => {
    ...
    setTabs(prev => prev.map(t => t.id === activeTab
      ? { ...t, subtabs: [...t.subtabs, { id, label: name, tasks: [], reminders: [] }] }
      : t
    ));
    setNewSubInput(""); setShowNewSub(false);
    setActiveSubtab(id);
  };

  const deleteSubtab = (id) => {
    setTabs(prev => prev.map(t => t.id === activeTab
      ? { ...t, subtabs: t.subtabs.filter(s => s.id !== id) }
      : t
    ));
  ...
  const addTask = () => {
    ...
    if (!text) return;
    updateCtx(c => ({ ...c, tasks: [...c.tasks, { id: uid(), text, done: false, createdAt: today() }] }));
    setTaskInput("");
  };
  ...
  const addReminder = () => {
    ...
    updateCtx(c => ({
      ...
      reminders: [...c.reminders, {
        id: uid(), text, done: false, createdAt: today(),
        startDate: reminderStart || null,
  ...
  const toggleDone = (type, id) => {
    ...
    updateCtx(c => ({ ...c, [key]: c[key].map(i => i.id === id ? { ...i, done: !i.done } : i) }));
  };

  const deleteItem = (type, id) => {
    const key = type === "task" ? "tasks" : "reminders";
    updateCtx(c => ({ ...c, [key]: c[key].filter(i => i.id !== id) }));
  };

  const saveEdit = (type, id) => {
    const key = type === "task" ? "tasks" : "reminders";
    updateCtx(c => ({ ...c, [key]: c[key].map(i => i.id === id ? { ...i, text: editText } : i) }));
  ...
  const sortedReminders = (ctx?.reminders || []).filter(r => !r.done).sort((a, b) => {
    ...
    return (order[getReminderStatus(a.startDate, a.endDate)] ?? 2) - (order[getReminderStatus(b.startDate, b.endDate)] ?? 2)
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .sub-chip {
          ...
          background: none; cursor: pointer;
          font-family: 'Heebo', sans-serif; font-size: 13px; color: #888;
        ...
        .plain-input {
          ...
          direction: rtl; transition: border 0.15s;
        }
        ...
        .ghost-btn {
          border: 1.5px dashed #d0d0ce; border-radius: 8px;
          background: none; padding: 7px 14px;
          font-family: 'Heebo', sans-serif; font-size: 13px; color: #aaa;
        ...
        .reminder-card {
          ...
          border: 1.5px solid #ebebea; background: white;
        ...
        .icon-btn {
          ...
          color: #ccc; font-size: 13px; padding: 2px 4px;
        ...
        .edit-inline {
          ...
          padding: 3px 8px; font-family: 'Heebo', sans-serif;
          font-size: 14px; outline: none; direction: rtl;
          background: white; color: #1a1a1a; width: 100%;
        }
        .days-badge {
          font-size: 11px; padding: 1px 7px; border-radius: 12px; font-weight: 600;
        }
      `}</style>

      <div style={{ "--accent": accent }}>
        <div style={{ background: "white", borderBottom: "1px solid #ebebea", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, paddingTop: 16, paddingBottom: 4 }}>
            <span style={{ fontSize: 20, fontWeight: 700, color: "#1a1a1a", marginLeft: 12 }}>לוח המשימות</span>
              ...
              >
                ...
                {activeTab === t.id && (
                  <span
                    ...
                    style={{ fontSize: 11, marginRight: -2, padding: 0 }}
        ...
        {currentTab && (
          <div style={{ padding: "20px 24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
              <button className={`sub-chip${!activeSubtab ? " active" : ""}`} style={{ "--accent": accent }} onClick={() => setActiveSubtab(null)}>
                כללי
              </button>
              {currentTab.subtabs.map(s => (
                <div key={s.id} style={{ position: "relative" }}>
                  <button className={`sub-chip${activeSubtab === s.id ? " active" : ""}`} style={{ "--accent": accent }}
                    onClick={() => setActiveSubtab(s.id)}>
                    {s.label}
                  </button>
                  {activeSubtab === s.id && (
                    <button className="icon-btn del"
                      style={{ position: "absolute", top: -5, left: -5, fontSize: 10, background: "#f5f5f4", borderRadius: "50%", width: 15, height: 15, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, alignItems: "start" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ fontWeight: 700, fontSize: 16, color: "#1a1a1a" }}>משימות</span>
                  {pendingTasks.length > 0 && (
                    <span style={{ marginRight: 8, background: accent, color: "white", borderRadius: 10, fontSize: 11, fontWeight: 700, padding: "1px 7px" }}>{pendingTasks.length}</span>
                  )}
                </div>

                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  <input className="plain-input" style={{ flex: 1 }}
                ...
                {pendingTasks.length === 0 && (
                  <div style={{ color: "#ccc", fontSize: 13, textAlign: "center", padding: "24px 0", border: "1px dashed #eee", borderRadius: 8 }}>
                    אין משימות פתוחות
                  </div>
                )}
                ...
                {pendingTasks.map(item => (
                  <div key={item.id} className="task-row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      {editId === item.id
                        ? <input autoFocus className="edit-inline"
                            value={editText}
                  ...
                  <>
                    ...
                    {doneTasks.map(item => (
                      <div key={item.id} className="task-row" style={{ opacity: 0.45 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <span style={{ fontSize: 14, textDecoration: "line-through", color: "#888" }}>{item.text}</span>
                        </div>
                        <button className="icon-btn del" onClick={() => deleteItem("task", item.id)}>✕</button>
                      </div>
                    ))}
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
                  <span style={{ fontWeight: 700, fontSize: 16, color: "#1a1a1a" }}>תזכורות</span>
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", gap: 8, marginBottom: showReminderDates ? 8 : 0 }}>
                    ...
                    />
                    <button
                      onClick={() => setShowReminderDates(p => !p)}
                      style={{ border: `1.5px solid ${showReminderDates ? accent : "#e5e5e3"}`, borderRadius: 8, background: "white", padding: "0 10px", cursor: "pointer", fontSize: 15, color: showReminderDates ? accent : "#aaa", transition: "all 0.15s" }}
                      title="הוסף תאריכים"
                    >📅</button>
                  ...
                  {showReminderDates && (
                    <div style={{ display: "flex", gap: 8, alignItems: "center", background: "#f9f9f8", border: "1px solid #ebebea", borderRadius: 8, padding: "10px 12px" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: 3, flex: 1 }}>
                        ...
                        <input type="date" className="plain-input" style={{ fontSize: 13, padding: "6px 10px", colorScheme: "light" }}
                          value={reminderStart}
                ...
                {sortedReminders.map(item => {
                  ...
                  const daysUntilStart = item.startDate ? getDaysUntil(item.startDate) : null;
                  const daysUntilEnd = item.endDate ? getDaysUntil(item.endDate) : null;
                  ...
                  return (
                    <div key={item.id} className={cardClass} style={{ "--accent": accent }}>
                      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                        <button className={`check-circle${item.done ? " checked" : ""}`} onClick={() => toggleDone("reminder", item.id)}>
                          {item.done ? "✓" : ""}
                        </button>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          {editId === item.id
                            ? <input autoFocus className="edit-inline"
                              ...
                              />
                            : <span style={{ fontSize: 14, fontWeight: 500, lineHeight: 1.4, color: "#1a1a1a" }}>{item.text}</span>
                          }
                            ...
                            <div className="date-range-bar" style={{ marginTop: 8 }}>
                              {status !== "none" && (
                                <>
                                  ...
                                  <span style={{ fontSize: 11, color: statusColor, fontWeight: 600, marginLeft: 2 }}>{statusLabel}</span>
                                  <span style={{ color: "#ddd" }}>·</span>
                                </>
                              )}
                              {item.startDate && (
                                <span className="date-chip" style={{ background: status === "active" ? `${accent}18` : "#f0f0ef", color: status === "active" ? accent : "#666" }}>
                              ...
                              {status === "future" && daysUntilStart !== null && (
                                <span className="days-badge" style={{ background: "#e8eaf6", color: "#5c6bc0", marginRight: 4 }}>
                                  בעוד {daysUntilStart} יום
                                </span>
                              )}
                              {status === "active" && daysUntilEnd !== null && (
                                <span className="days-badge" style={{ background: `${accent}18`, color: accent, marginRight: 4 }}>
                                  {daysUntilEnd === 0 ? "מסתיים היום" : `${daysUntilEnd} יום נותרו`}
                                </span>
                              )}
                              {status === "active" && !item.endDate && (
                                <span className="days-badge" style={{ background: `${accent}18`, color: accent, marginRight: 4 }}>פעיל</span>
                              )}
                            </div>
                          )}
                        </div>
